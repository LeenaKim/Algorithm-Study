...
public class Naver2020_1 {
	...
	public static String solution(int n, int[] p, int[] c) {
		...
		loop: for(int i = 0; i < n; i++) {
			...
			if(p[i] >= c[i]) {
				System.out.println("재고 충분 | 재고 : " + p[i] + ", 주문 : " + c[i]);
				if(i < n-1) {
					p[i+1] += p[i] - c[i];
				} 
				income += c[i] * price[priceIdx];
				priceIdx = 0;					
			} else {
				System.out.println("재고 없음 | 재고 : " + p[i] + ", 주문 : " + c[i]);
				if(i < n-1) {
					p[i+1] += p[i];					
				} 
				if(priceIdx == 2) {
		...
		answerFloat = Math.round(income/cnt * 100) / 100;
		System.out.println("answerFloat : " + answerFloat);
		answer = String.format("%.2f", answerFloat);
		System.out.println("income average : " + answer);
		return answer;
	}
